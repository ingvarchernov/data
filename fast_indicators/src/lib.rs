...
mod tests {
    ...
    fn test_rsi_basic() {
        ...
        for i in 1..prices.len() {
            ...
            if change > 0.0 {
                gains.push(change);
                losses.push(0.0);  // No loss on this period
            } else {
                gains.push(0.0);   // No gain on this period
                losses.push(change.abs());  // Store as positive value
            }
        }

        // Add assertions to verify the logic
        assert_eq!(gains.len(), prices.len() - 1);
        assert_eq!(losses.len(), prices.len() - 1);

        // Verify that gains and losses are mutually exclusive
        for i in 0..gains.len() {
            assert!(gains[i] == 0.0 || losses[i] == 0.0);
        }
    }
}
